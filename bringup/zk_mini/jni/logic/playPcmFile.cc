...
static void onUI_init() {
    ...
    /*parseVideoFileList("/mnt/extsd/videolist.txt", sMediaFileList);
...
static void onUI_intent(const Intent *intentPtr) {
    parseVideoFileList("/mnt/extsd/video2_video_list.txt", sMediaFileList);
#if 0
    mVideoviewTTPtr->setVolume(0.5);
#endif
    sPlayIndex = -1;
...
static bool onUI_Timer(int id) {
    ...
    case 1: {
        ...
        int curPos = -1;
        if (sAudioPlayerPtr) {
            if (sAudioPlayerPtr->isPlaying()) {
                curPos = sAudioPlayerPtr->getCurrentPosition() / 1000;
            }
        } else {
            if (mVideoviewTTPtr->isPlaying()) {
...
//    if (sAudioPlayerPtr) {
//        sAudioPlayerPtr->setVolume((float) progress / 10, (float) progress / 10);
//    } else {
...
static bool onButtonClick_PlayButton(ZKButton *pButton) {
    ...
    if (sAudioPlayerPtr) {
        if (sAudioPlayerPtr->isPlaying()) {
            sAudioPlayerPtr->pause();
            pButton->setSelected(false);
        } else {
            sAudioPlayerPtr->resume();
