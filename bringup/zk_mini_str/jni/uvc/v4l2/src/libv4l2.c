...
static int v4l2_map_buffers(int index)
{
  ...
  for (i = 0; i < devices[index].no_frames; i++) {
    ...
    devices[index].frame_pointers[i] = (unsigned char*)SYS_MMAP(NULL,
...
static void v4l2_unmap_buffers(int index)
{
  ...
  for (i = 0; i < devices[index].no_frames; i++) {
    if (devices[index].frame_pointers[i] != MAP_FAILED) {
      ...
      devices[index].frame_pointers[i] = (unsigned char*)MAP_FAILED;
...
static int v4l2_read_and_convert(int index, unsigned char *dest, int dest_size)
{
  ...
  if (devices[index].readbuf_size < buf_size) {
    ...
    new_buf = (unsigned char*)realloc(devices[index].readbuf, buf_size);
    if (!new_buf)
      return -1;
...
int v4l2_fd_open(int fd, int v4l2_flags)
{
  ...
  devices[index].convert_mmap_buf = (unsigned char*)MAP_FAILED;
...
int v4l2_ioctl (int fd, unsigned long int request, ...)
{
  ...
  switch (request) {
    ...
    case VIDIOC_ENUM_FMT:
      result = v4lconvert_enum_fmt(devices[index].convert, (struct v4l2_fmtdesc*)arg);
      break;

    case VIDIOC_ENUM_FRAMESIZES:
      result = v4lconvert_enum_framesizes(devices[index].convert, (struct v4l2_frmsizeenum *)arg);
      break;

    case VIDIOC_ENUM_FRAMEINTERVALS:
      result = v4lconvert_enum_frameintervals(devices[index].convert, (struct v4l2_frmivalenum *)arg);
      if (result)
    ...
    case VIDIOC_TRY_FMT:
      result = v4lconvert_try_format(devices[index].convert, (struct v4l2_format *)arg, NULL);
      break;

    case VIDIOC_S_FMT:
      {
	struct v4l2_format src_fmt, *dest_fmt = (struct v4l2_format*)arg;
	...
	*fmt = devices[index].dest_fmt;
	...
	/* An application can do a DQBUF before mmap-ing in the buffer,
	   ...
	   to it! */
	if (devices[index].convert_mmap_buf == MAP_FAILED) {
	  devices[index].convert_mmap_buf = (unsigned char*)SYS_MMAP(NULL,
						   (size_t)(
...
ssize_t v4l2_read (int fd, void* dest, size_t n)
{
  ...
  if (devices[index].flags & V4L2_USE_READ_FOR_READ) {
    result = v4l2_read_and_convert(index, (unsigned char*)dest, n);
  } else {
    struct v4l2_buffer buf;

    buf.type   = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    buf.memory = V4L2_MEMORY_MMAP;
    result = v4l2_dequeue_and_convert(index, &buf, (unsigned char*)dest, n);
