...
player_stat_t *player_init(const char *p_input_file)
{
    ...
    int ret = pthread_cond_init(&is->continue_read_thread, NULL);
    if (ret != 0) {
        goto fail;
    }
...
int player_deinit(player_stat_t *is)
{
    ...
    if (true == is->demux_status)
    {
        ...
        if (is->audio_idx >= 0)
        {
            ...
            if (is->functions.audio_deinit && is->audio_enable) {
                is->functions.audio_deinit(is);
                printf("player audio deinit done\n");
            }
        }
        ...
        if (is->video_idx >= 0)
        {
            ...
            if (is->functions.video_deinit && is->video_enable) {
                is->functions.video_deinit(is);
                printf("player video deinit done\n");
            }
        }
        ...
        printf("avformat_network_deinit finish\n");
    }
