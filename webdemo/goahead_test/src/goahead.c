...
void work(void *Pfunc)
{
	...
	char *argv[1];
	argv[0] = "./goahead";
	registerWebFun = (void (*)())Pfunc;
    char    *argp, *home, *documents, *endpoints, *endpoint, *route, *auth, *tok, *lspec;
    int     argind;

	/*char *envp[7];
	envp[0] = "LD_LIBRARY_PATH=/lib:/system/lib:/vendor/lib:/ext/lib";
	envp[1] = "SHLVL=2";
	envp[2] = "OLDPWD=/ext/demo";
	envp[3] = "PS1=[\w]## ";
	envp[4] = "PATH=/sbin:/system/sbin:/system/bin:/vendor/bin:/ext/bin";
	envp[5] = "ANDROID_PROPERTY_WORKSPACE=8,49152";
	envp[6] = "PWD=/vendor";*/
	/*printf("argc=%d\n", argc);
	printf("argv=%s\n", argv[0]);
	printf("envp0=%s\n", envp[0]);
	...
	documents = webFilesPath;//ÉèÖÃÍøÒ³Ä¿Â¼
    initPlatform();
