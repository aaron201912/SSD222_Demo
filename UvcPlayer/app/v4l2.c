...
static int new_packet(Packet *pkt, int size)
{
    ...
    buf = v4l2_malloc(sizeof(BufferRef));
    if(!buf)
        return -ENOMEM;

    buf->size = size;
    buf->data = v4l2_malloc(size * sizeof(int));
    if(!buf->data) {
        v4l2_free(buf);
        return -ENOMEM;
    }
...
static int mmap_init(DeviceContex_t *ctx)
{
    ...
    s->buffers = req.count;
    s->buf_start = v4l2_malloc(s->buffers * sizeof(void *));
    if (!s->buf_start) {
        DEMO_ERR("Cannot allocate buffer pointers\n");
        return -ENOMEM;
    }
    s->buf_len = v4l2_malloc(s->buffers * sizeof(unsigned int));
    if (!s->buf_len) {
...
int v4l2_dev_init(DeviceContex_t **ctx, char *path)
{
    ...
    DeviceContex_t *c = 
       v4l2_malloc(sizeof(DeviceContex_t));
    struct video_data *s = 
       v4l2_malloc(sizeof(struct video_data));
